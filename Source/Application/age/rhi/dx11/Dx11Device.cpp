...
namespace Ag::rhi::dx11
{
	...
	{
		myDevice = Ag::DX11::Device;
		myCtx    = Ag::DX11::Context;
		assert(myDevice && myCtx && "Ag::DX11 must be Init()ed before rhi::CreateDevice(DX11)");
	...
	void Dx11Device::AdoptSwapchainResources()
	{
		...
		RenderTarget* bb  = Ag::DX11::BackBuffer;
		RenderTarget* bbL = Ag::DX11::BackBufferNoSrgbConversion;
		Ag::DepthBuffer* db = Ag::DX11::DepthBuffer;
		...
		// Backbuffer texture handle (resource not owned here).
		{
			...
			const Vector2ui r = Ag::DX11::GetResolution();
	...
	Vector2ui Dx11Device::GetResolution() const { return Ag::DX11::GetResolution(); }

	bool Dx11Device::Resize(uint32_t, uint32_t)
	{
		// Stage 1: the actual swapchain resize is still driven by Ag::DX11.
	...
	{
		// Stage 1: Ag::DX11::EndFrame still owns Present. When DX11.cpp becomes a
