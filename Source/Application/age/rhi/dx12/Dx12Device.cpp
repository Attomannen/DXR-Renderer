...
namespace Ag::rhi::dx12
{
	...
	Dx12Device::Dx12Device(const DeviceDesc& d)
	{
		...
		{ AG_CPU_SCOPE("Descriptor heaps"); CreateHeaps(); CreateNullDescriptors(); }
		{ AG_CPU_SCOPE("Frame resources"); CreateFrameResources(); }
		{ AG_CPU_SCOPE("Root signatures"); CreateRootSignatures(); }
		if (myHwnd) { AG_CPU_SCOPE("Swap chain"); CreateSwapchain(myHwnd, d.width, d.height); }
	...
	void Dx12Device::CreateDeviceAndQueue(bool enableDebugLayer, bool enableGpuValidation)
	{
		...
		AG_CPU_SCOPE("Factory + adapter + D3D12CreateDevice");
		HRESULT hr;
		{ AG_CPU_SCOPE("CreateDXGIFactory2");
		...
		for (UINT i = 0;; ++i)
		{
			{ AG_CPU_SCOPE("EnumAdapterByGpuPreference");
			...
			bool created;
			{ AG_CPU_SCOPE("D3D12CreateDevice");
	...
	void Dx12Device::EndFrame(bool vsync)
	{
		...
		// Cheap no-op unless AGE_DX12_DEBUG_LAYER actually enabled the debug
	...
	BufferHandle Dx12Device::CreateBufferImpl(const BufferDesc& d, const std::function<void(void*)>* fill)
	{
		...
		ComPtr<ID3D12Resource> res;
		HRESULT hr;
		{ AG_CPU_SCOPE("Buffer: CreateCommittedResource");
	...
	TextureHandle Dx12Device::CreateTexture(const TextureDesc& d, const SubresourceData* initial, uint32_t initialCount)
	{
		...
		ComPtr<ID3D12Resource> res;
		HRESULT hr;
		{ AG_CPU_SCOPE("Texture: CreateCommittedResource");
	...
	{
		AG_CPU_SCOPE("Buffer: staging fill + copy");
	...
	void Dx12Device::UploadTextureData(ID3D12Resource* dst, const TextureDesc& d, const SubresourceData* initial, uint32_t count)
	{
		...
		AG_CPU_SCOPE("Texture: staging fill + copy");
