...
struct AudioData {
	...
	std::unordered_map<Ag::StringId, struct ma_sound> soundCache;
} static locAudioData;
...
void Audio::Init(const char * aPath, Ag::StringId aKey, bool anPlayOnLoad, bool aIsLooping)
{
	ma_result result;
...
void Audio::Play(Ag::StringId aKey, bool aResetAndPlay)
{
...
bool Audio::IsPlaying(Ag::StringId aKey)
	{
	return (bool)ma_sound_is_playing(&locAudioData.soundCache[aKey]);
	}

void Ag::Audio::SetVolume(Ag::StringId aKey, float aVolume)
{
	ma_sound* sound = &locAudioData.soundCache[aKey];
	myVolume = aVolume;
...
void Ag::Audio::SetPosition(Ag::StringId aKey, Vector3f aPosition)
{
	ma_sound* sound = &locAudioData.soundCache[aKey];
...
void Audio::Stop(Ag::StringId aKey, bool aImmediately)
{
	ma_sound* sound = &locAudioData.soundCache[aKey];
	if (aImmediately)
	{
