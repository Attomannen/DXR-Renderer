...
#include <age/drawers/DebugPerformancegraph.h>
...
void DebugDrawer::Init()
{
	...
	myFPS = std::make_unique<Ag::Text>("Text/arial.ttf", FontSize_24);
	myFPS->SetText("--");
	myFPS->SetColor({ 1, 1, 1, 1.0f });

	myMemUsage = std::make_unique<Ag::Text>("Text/arial.ttf");
	myMemUsage->SetText("--");
	myMemUsage->SetColor({ 1, 1, 1, 1.0f });

	myDrawCallText = std::make_unique<Ag::Text>("Text/arial.ttf");
	myDrawCallText->SetText("--");
	myDrawCallText->SetColor({ 1, 1, 1, 1.0f });

	myCPUText = std::make_unique<Ag::Text>("Text/arial.ttf");
	myCPUText->SetText("--");
	myCPUText->SetColor({ 1, 1, 1, 1.0f });

	myLogText = std::make_unique<Ag::Text>("Text/arial.ttf");
	myLogText->SetText("--");
	...
	Ag::Color bgColor(0, 1, 0, 0.4f);
	Ag::Color lineColro(1, 1, 1, 1.0f);
...
void DebugDrawer::Update(float aTimeDelta)
{
	...
	Vector2ui intResolution = Ag::Application::GetInstance()->GetRenderSize();
	...
	if (myShowErrorTimer > 0.0f)
	{
		if (!myErrorTexture->myIsFailedTexture)
		{
			...
			instanceData.color = Ag::Color(1, 1, 1, std::min(myShowErrorTimer, 1.0f));

			Ag::GraphicsEngine::GetInstance()->GetSpriteDrawer().Draw(sharedData, instanceData);
...
void Ag::DebugDrawer::DrawLine(Vector2f aFrom, Vector2f aTo, Color aColor)
{
...
void DebugDrawer::DrawCircle(Vector2f aPos, float aRadius, Color aColor)
{
	...
	LineDrawer& lineDrawer = Ag::GraphicsEngine::GetInstance()->GetLineDrawer();
