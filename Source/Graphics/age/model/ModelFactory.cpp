#include "stdafx.h"
#include "age/model/MeshSimplify.h"
#include <atomic>
...
void AssignDefaultMaterials(std::string_view someFilePath, Model* aModel)
{
	...
	for (int i = 0; i < aModel->GetMeshCount(); i++)
	{
		...
		if (std::getenv("AGE_LOG_MATERIALS"))
		{
...
namespace
{
	...
	Ag::BoxSphereBounds BoundsOf(const std::vector<Ag::Vertex>& v)
	{
		Ag::Vector3f mn{ 1e30f, 1e30f, 1e30f }, mx{ -1e30f, -1e30f, -1e30f };
		for (const Ag::Vertex& vert : v)
		{
	...
	bool CacheCreateBuffers(Ag::Model::MeshData& md, Ag::Model::VertexFormat format)
	{
		md.vertexBuffer = {};
		md.indexBuffer  = {};
		...
		Ag::rhi::IDevice* dev = Ag::DX11::Rhi();
		if (!Ag::Model::CreateVertexBuffer(md, md.vertices.data(), (uint32_t)md.vertices.size(), format, "Mesh_VB"))
		{
			ERROR_PRINT("mesh '%s': vertex buffer create failed (%zu verts)", md.name.GetString(), md.vertices.size());
			return false;
		}
		const Ag::rhi::BufferHandle vb = md.vertexBuffer;

		Ag::rhi::BufferDesc ibd{};
		ibd.byteSize = (UINT)(md.indices.size() * sizeof(unsigned int));
		ibd.stride = sizeof(unsigned int);
		ibd.usage = Ag::rhi::BufferUsage::Index | Ag::rhi::BufferUsage::ByteAddress;
		ibd.memory = Ag::rhi::MemoryType::Default;
		ibd.debugName = "Mesh_IB";
		Ag::rhi::BufferHandle ib = dev->CreateBuffer(ibd, md.indices.data());
	...
	void MergeMeshesByMaterial(std::vector<Ag::Model::MeshData>& meshes, Ag::Model::VertexFormat format)
	{
		...
		if (!kMergeByMaterial || meshes.size() < 2)
		{
			...
			for (Ag::Model::MeshData& md : meshes)
			{
		...
		for (Ag::Model::MeshData& m : meshes)
		{
			...
			{
				Ag::Model::MeshData& dst = out[it->second];
	...
	bool VertexIsCompactColorSafe(const Ag::Vertex& v)
	{
		auto nz2 = [](const Ag::Vector2f& a) { return a.x != 0.f || a.y != 0.f; };
		auto nz4 = [](const Ag::Vector4f& a) { return a.x != 0.f || a.y != 0.f || a.z != 0.f || a.w != 0.f; };
	...
	bool VertexIsCompactSafe(const Ag::Vertex& v)
	{
		auto nz2 = [](const Ag::Vector2f& a) { return a.x != 0.f || a.y != 0.f; };
		auto nz4 = [](const Ag::Vector4f& a) { return a.x != 0.f || a.y != 0.f || a.z != 0.f || a.w != 0.f; };
	...
	std::string MeshCachePath(const char* assetPath)
	{
		...
		std::string root = Ag::Settings::CookedAssetRoot();
		if (root.empty()) root = ".";
	...
	// AGE_LOD_RATIO=<0..1> decimates every cached mesh on load, to measure what a
	...
	float DebugLodRatio()
	{
		static const float ratio = [] {
			const char* v = std::getenv("AGE_LOD_RATIO");
	...
	void DecodeVertices(uint8_t layout, const uint8_t* src, uint32_t count, Ag::MeshVertex* dst, bool allowThreads = true)
	{
		...
		auto decode = [=](uint32_t first, uint32_t last)
		{
			for (uint32_t v = first; v < last; ++v)
			{
				...
				{
					Ag::Vertex full;
					memcpy(&full, src + (size_t)v * sizeof(Ag::Vertex), sizeof(Ag::Vertex));
					dst[v] = Ag::PackMeshVertex(full);
					continue;
				}
	...
	bool TryLoadMeshCache(const std::string& cachePath, const char* fbxPath,
	                      Ag::Model* outModel, const std::string& modelPathForInit)
	{
		AG_CPU_SCOPE("Mesh cache read");
		...
		Ag::rhi::IDevice* dev = Ag::DX11::Rhi();
		std::vector<Ag::Model::MeshData> meshes(meshCount);
		...
		struct PendingMesh
		{
			...
			Ag::MeshVertex* decoded = nullptr;   // into decodeArena below
		};
		std::vector<PendingMesh> pending;
		pending.reserve(meshCount);
		for (uint32_t m = 0; m < meshCount; ++m)
		{
			Ag::Model::MeshData& md = meshes[m];
			md.name         = Ag::StringRegistry::RegisterOrGetString(in.ReadStr());
			md.materialName = Ag::StringRegistry::RegisterOrGetString(in.ReadStr());
			md.bounds = in.Read<Ag::BoxSphereBounds>();

			const uint8_t layout = in.Read<uint8_t>();   // 0/2 legacy float forms, 1 full, 3 pre-packed
			const uint32_t vc = in.Read<uint32_t>();
			const uint64_t vertexBytes = layout == 3 ? (uint64_t)vc * sizeof(Ag::MeshVertex)
				: layout == 1 ? (uint64_t)vc * sizeof(Ag::Vertex)
			...
			Ag::Model::SetVertexFormat(md, Ag::Model::VertexFormat::Compact);
			...
			{
				Ag::MeshSimplifySource src;
				const uint32_t floats = layout == 2 ? kCompactColorFloats : kCompactFloats;
				src.vertexCount = vc;
				if (layout == 1)
				{
					src.positions = vertexData + offsetof(Ag::Vertex, position);
					src.positionStride = sizeof(Ag::Vertex);
					src.uv0 = vertexData + offsetof(Ag::Vertex, uvs);
					src.uvStride = sizeof(Ag::Vertex);
				}
				else
				{
				...
				const Ag::MeshSimplifyResult lod = Ag::SimplifyMesh(src,
		...
		std::unique_ptr<Ag::MeshVertex[]> decodeArena(totalVertices ? new Ag::MeshVertex[totalVertices] : nullptr);
		{
			uint64_t offset = 0;
			for (PendingMesh& pm : pending) { if (pm.layout == 3) continue; pm.decoded = decodeArena.get() + offset; offset += pm.vertexCount; }
		}
		{
			AG_CPU_SCOPE("Mesh vertex decode (parallel)");
			const uint32_t threads = std::clamp(std::thread::hardware_concurrency(), 1u, 16u);
		...
		{
			AG_CPU_SCOPE("Mesh GPU buffers");
			for (PendingMesh& pm : pending)
			{
				Ag::Model::MeshData& md = meshes[pm.mesh];
				// The cache only holds static meshes, which always upload compact.
				Ag::Model::CreateVertexBuffer(md, pm.vertexCount,
					[&](Ag::MeshVertex* mapped)
					{
						const void* src = pm.layout == 3 ? static_cast<const void*>(pm.vertexData) : static_cast<const void*>(pm.decoded);
						memcpy(mapped, src, (size_t)pm.vertexCount * sizeof(Ag::MeshVertex));
					}, "Mesh_VB");
				const Ag::rhi::BufferHandle vb = md.vertexBuffer;
				...
				ibd.memory = Ag::rhi::MemoryType::Default;
				ibd.debugName = "Mesh_IB";
				const Ag::rhi::BufferHandle ib = vb.IsValid() ? dev->CreateBuffer(ibd, pm.indexData) : Ag::rhi::BufferHandle{};
		...
		AG_CPU_SCOPE("Model init");
		outModel->Init(std::move(meshes), modelPathForInit);
		return true;
	}

	bool ReadCacheGeometry(const std::string& cachePath, const char* fbxPath, Ag::CollisionGeometry& out)
	{
		int64_t fbxTime = 0; uint64_t fbxSize = 0;
		if (!CacheFileStamp(fbxPath, fbxTime, fbxSize)) return false;
		...
		for (uint32_t m = 0; m < meshCount; ++m)
		{
			...
			const uint64_t stride = layout == 3 ? sizeof(Ag::MeshVertex)
				: layout == 1 ? sizeof(Ag::Vertex) : (uint64_t)floats * sizeof(float);
			...
			for (uint32_t v = 0; v < vc; ++v)
			{
				...
				float p[3];
				memcpy(p, vertexData + (size_t)v * stride + (layout == 1 ? offsetof(Ag::Vertex, position) : 0), sizeof(p));
	...
	void WriteMeshCache(const std::string& cachePath, const char* fbxPath,
	                    const std::vector<Ag::Model::MeshData>& meshes)
	{
		int64_t fbxTime = 0; uint64_t fbxSize = 0;
		...
		std::vector<Ag::MeshVertex> packed;
		for (const auto& md : meshes)
		{
			...
			for (const Ag::Vertex& v : md.vertices)
			{
			...
			// Layout 1 (full Ag::Vertex) stays, for meshes that are not.
			...
			{
				packed.resize(vc);
				for (uint32_t v = 0; v < vc; ++v) packed[v] = Ag::PackMeshVertex(md.vertices[v]);
				if (vc) out.write(reinterpret_cast<const char*>(packed.data()), (std::streamsize)vc * sizeof(Ag::MeshVertex));
			}
			else if (layout != 1)
			{
				const uint32_t floats = layout == 2 ? kCompactColorFloats : kCompactFloats;
				...
				for (uint32_t v = 0; v < vc; ++v)
				{
					const Ag::Vertex& s = md.vertices[v];
			...
			{
				out.write(reinterpret_cast<const char*>(md.vertices.data()), (std::streamsize)vc * sizeof(Ag::Vertex));
			}
...
static void GenerateTangents(std::vector<Vertex>& verts, const std::vector<uint32_t>& indices)
{
	...
	for (size_t i = 0; i < indices.size(); i += 3)
	{
		...
		Ag::Vector3f dp1 = p1 - p0;
		Ag::Vector3f dp2 = p2 - p0;

		Ag::Vector2f duv1 = uv1 - uv0;
		Ag::Vector2f duv2 = uv2 - uv0;
		...
		Ag::Vector3f tangent = (dp1 * duv2.y - dp2 * duv1.y) * r;
		Ag::Vector3f bitangent = (dp2 * duv1.x - dp1 * duv2.x) * r;
	...
	for (auto& v : verts)
	{
		Ag::Vector3f n = v.normal;
		Ag::Vector3f t = v.tangent;

		if (t.LengthSqr() < 1e-6f)
		{
			// fallback: build arbitrary tangent to avoid broken meshes
			Ag::Vector3f up = fabs(n.z) < 0.999f ? Ag::Vector3f{ 0,0,1 } : Ag::Vector3f{ 0,1,0 };
...
bool ModelFactory::GetCollisionGeometry(StringId someFilePath, CollisionGeometry& outGeometry)
{
	...
	if (someFilePath.IsEmpty() || !Ag::Settings::ResolveAssetPath(someFilePath, resolved_path))
		return false;
	...
	const std::shared_ptr<const Skeleton>& aSkeleton)
{
	...
	if (!Ag::Settings::ResolveAssetPath(someFilePath, resolvedPath))
		return nullptr;
...
namespace
{
	struct CpuImportResult
	{
		...
		bool skinned = false;
		std::string error;
		std::vector<Ag::Model::MeshData> meshes;
...
void ModelFactory::RequestAsyncImport(StringId path)
{
	...
	if (TryLoadMeshCache(cachePath, resolved.GetData(), cached.get(), resolvedPath))
	{
		...
		if (myWatchedPaths.insert(path).second)
			Application::GetInstance()->GetFileWatcher()->WatchFileChange(resolved.GetStringView(), std::bind(&Ag::ModelFactory::OnModelChanged, this, path));
...
void ModelFactory::PumpAsyncImports()
{
	...
	for (const auto& job : ready)
	{
		...
		if (myWatchedPaths.insert(job->path).second)
			Application::GetInstance()->GetFileWatcher()->WatchFileChange(job->resolvedPath, std::bind(&Ag::ModelFactory::OnModelChanged, this, job->path));
...
std::shared_ptr<Model> ModelFactory::LoadModel(StringId someFilePath)
{
	...
	if (myWatchedPaths.find(someFilePath) == myWatchedPaths.end())
	{
		...
		Application::GetInstance()->GetFileWatcher()->WatchFileChange(resolved_path.GetStringView(), std::bind(&Ag::ModelFactory::OnModelChanged, this, someFilePath));
	}
...
std::shared_ptr<const Animation> ModelFactory::GetAnimation(StringId someFilePath, const std::shared_ptr<const Skeleton>& aSkeleton)
{
	...
	if (TGA::FBX::Importer::LoadAnimationA(resolvedPath.GetData(), fbxAnimation))
	{
		...
		const Ag::Skeleton& skeleton = *aSkeleton;
...
Ag::BoxSphereBounds Ag::ModelFactory::CalculateBoxSphereBounds(std::vector<Ag::Vertex> somePositions)
{
