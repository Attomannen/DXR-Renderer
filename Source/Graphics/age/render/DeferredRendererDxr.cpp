...
void DeferredRenderer::MeasureEnvironment(rhi::ICommandContext& ctx)
{
	...
	AG_PROFILE_SCOPE(myProfiler, "Environment measure");
...
void DeferredRenderer::GatherEmissiveLights(rhi::ICommandContext& ctx)
{
	...
	AG_PROFILE_SCOPE(myProfiler, "Emissive light gather");
...
void DeferredRenderer::RenderDxrLighting()
{
	...
	{
		AG_PROFILE_SCOPE(myProfiler, "Material table upload");
...

	{
		...
		AG_PROFILE_SCOPE(myProfiler, "Ray trace + shade");
...
void DeferredRenderer::DenoiseDxrDiffuse(rhi::ICommandContext& ctx)
{
	...
	{
		AG_PROFILE_SCOPE(myProfiler, "NRD denoise");
		dx12Ctx->PushMarker("NRD RELAX diffuse+specular");
		myNrd->Denoise(*dx12Ctx, in, settings);
		dx12Ctx->PopMarker();
	}

	AG_PROFILE_SCOPE(myProfiler, "NRD composite");
...
void DeferredRenderer::ResolveDxrLightingToHdr()
{
	...
	{
		AG_PROFILE_SCOPE(myProfiler, rrRequested ? "DLSS Ray Reconstruction" : "DLSS / DLAA");
	...
	if (resolveTemporal) {
		AG_PROFILE_SCOPE(myProfiler, "TAA resolve");
...
void DeferredRenderer::WriteDxrDepthToDepthBuffer()
{
	...
	AG_PROFILE_SCOPE(myProfiler, "DXR depth for transparents");
	rhi::ICommandContext& ctx = DX11::Rhi()->GetContext();
