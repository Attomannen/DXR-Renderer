...
namespace
{
	...
	struct ShadowCb
	{
		float cascadeViewProj[Ag::DeferredRenderer::kNumCascades][16];
	...
	inline Ag::Matrix4x4f MakeLookTransform(const Ag::Vector3f& pos, Ag::Vector3f fwd)
	{
		float l = std::sqrt(fwd.x*fwd.x + fwd.y*fwd.y + fwd.z*fwd.z);
		if (l > 1e-5f) fwd = { fwd.x/l, fwd.y/l, fwd.z/l };
		Ag::Vector3f up = (std::abs(fwd.y) > 0.99f) ? Ag::Vector3f{ 0,0,1 } : Ag::Vector3f{ 0,1,0 };
		Ag::Vector3f right{ up.y*fwd.z - up.z*fwd.y, up.z*fwd.x - up.x*fwd.z, up.x*fwd.y - up.y*fwd.x };
		...
		Ag::Matrix4x4f m = Ag::Matrix4x4f::CreateIdentityMatrix();
	...
	using DxrLightingConstants = Ag::DxrShared::DxrLightingConstants;
	...
	using SkyAtmosphereConstants = Ag::SkyShared::SkyAtmosphereConstants;
	...
	using CloudsConstants = Ag::CloudsShared::CloudsConstants;
	...
	inline Ag::Vector3f Lerp(const Ag::Vector3f& a, const Ag::Vector3f& b, float t)
	{
	...
	inline void SetTargets(Ag::rhi::ICommandContext& aCtx,
	                std::initializer_list<Ag::rhi::RtvHandle> aRtvs,
	                Ag::rhi::DsvHandle aDsv, Ag::Vector2ui aSize)
	{
		Ag::rhi::RtvHandle arr[8] = {};
		uint32_t n = 0;
		for (Ag::rhi::RtvHandle h : aRtvs) if (n < 8) arr[n++] = h;
