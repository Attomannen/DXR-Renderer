...
Font TextService::GetOrLoad(std::string aFontPathAndName, FontSize aFontSize, unsigned char aBorderSize)
{
	...
	if (!Ag::Settings::ResolveAssetPath(aFontPathAndName, resolvedPath))
	{
		return { nullptr };
	}
...
float Ag::TextService::GetSentenceWidth(Ag::Text& aText)
{
...
float Ag::TextService::GetSentenceHeight(Ag::Text& aText)
{
	if (aText.myText.size() <= 0)
	{
		return 0.0f;
...
bool TextService::Draw(Ag::Text& aText, Ag::SpriteShader* aCustomShader)
{
	aCustomShader;
	const InternalTextAndFontData* fontData = aText.myFont.myData.get();
