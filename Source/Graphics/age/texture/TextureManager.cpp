...
#include <vector>
#include <age/noise/PerlinNoise.h>
...
Texture* TextureManager::TryGetTexture(const char* aTexturePath, TextureSrgbMode aSrgbMode, bool aForceReload)
{
	...
	if (!loadedTexture)
		Application::GetInstance()->GetFileWatcher()->WatchFileChange(asset_path.GetStringView(), std::bind(&Ag::TextureManager::OnTextureChanged, this, stringId));
...
{
	AG_CPU_SCOPE("Texture prefetch (parallel read)");
...
{
	AG_CPU_SCOPE("Texture load (DX12)");
...
void Ag::TextureManager::ReleaseTexture(Texture* aTexture)
{
	rhi::IDevice* dev = DX11::Rhi();
	if (dev && dev->GetBackend() == rhi::Backend::DX12)
	{
