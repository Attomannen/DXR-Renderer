...
ed::EditorAction::AcceptResult ed::ContextMenuAction::Accept(const Control& control)
{
    ...
    // Locally patched: upstream used a 1px drag-lock threshold here, which
    // made an ordinary right-click frequently get misread as the start of a
    // drag (and so silently fail to open the context menu at all) on any
    // real mouse's natural hand tremor between press and release. 4px, this
    // repo's own tolerance, still comfortably distinguishes a click from an
    // intentional right-drag-to-pan gesture.
    const auto isDragging = ImGui::IsMouseDragging(Editor->GetConfig().ContextMenuButtonIndex, 4);
