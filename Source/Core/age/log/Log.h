...
#ifndef _RETAIL
	#define ERROR_PRINT(aFormat, ...) Ag::Log::LogError(__FILE__, __LINE__, aFormat, ##__VA_ARGS__)
	#define INFO_PRINT(aFormat, ...) Ag::Log::LogInfo(aFormat, ##__VA_ARGS__)
	#define INFO_TIP(aFormat, ...) Ag::Log::LogTip(aFormat, ##__VA_ARGS__)

#define ASSERT(condition, message, ...) \
		do { \
		if(!condition) \
		{ \
				Ag::Log::LogError(__FILE__, __LINE__, "Assertion '" #condition "' failed in %s! " #message, __FUNCTION__, ##__VA_ARGS__); \
			std::terminate(); \
		} \
		} while (0)
#else
