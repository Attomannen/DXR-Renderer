...
namespace FMath
{
	...
	template <class T>
	::Ag::Vector3<T> NLerp(const ::Ag::Vector3<T>& a, const ::Ag::Vector3<T>& b, float t)
	{
	...
	template <class T>
	::Ag::Vector3<T> SLerp(::Ag::Vector3<T> start, ::Ag::Vector3<T> end, float percent)
	{
		...
		::Ag::Vector3<T> RelativeVec = end - start * dot;
		RelativeVec.Normalize();
		// Orthonormal basis
		// The final result.
		::Ag::Vector3<T> returnValue= ((start * cos(theta)) + (RelativeVec * sin(theta)));
		return returnValue;
	}
}