...
bool Ag::CheckFrustum(const Frustum& frustum, Vector3f center, float radius)
{
	Ag::Vector3f FrustumToPoint{};
...
Frustum Ag::CalculateFrustum(const Camera& camera)
{
	const Ag::Matrix4x4f& projection = camera.GetProjection();
	const Ag::Matrix4x4f& cameraToWorld = camera.GetTransform();

	Ag::Vector3f tlc = Ag::Vector3f{
		(-1.0f) / projection(1,1),
		(1.0f) / projection(2,2),
		1.f
	}.GetNormalized();

	Ag::Vector3f brc = Ag::Vector3f{
		(1.0f) / projection(1,1),
		(-1.0f) / projection(2,2),
		1.f
	}.GetNormalized();

	Ag::Vector3f trc = Ag::Vector3f{
		(1.0f) / projection(1,1),
		(1.0f) / projection(2,2),
		1.f
	}.GetNormalized();

	Ag::Vector3f blc = Ag::Vector3f{
		(-1.0f) / projection(1,1),
		(-1.0f) / projection(2,2),
		1.f
	}.GetNormalized();
	...
	Ag::Vector3f tln = (nearPlane * tlc) * cameraToWorld;
	Ag::Vector3f brn = (nearPlane * brc) * cameraToWorld;
	Ag::Vector3f trn = (nearPlane * trc) * cameraToWorld;
	Ag::Vector3f bln = (nearPlane * blc) * cameraToWorld;

	Ag::Vector3f tlf = (farPlane * tlc) * cameraToWorld;
	Ag::Vector3f brf = (farPlane * brc) * cameraToWorld;
	Ag::Vector3f trf = (farPlane * trc) * cameraToWorld;
	Ag::Vector3f blf = (farPlane * blc) * cameraToWorld;
