...
#include <tge/editor/ScriptEditor/ScriptEditorNodeCatalog.h>
...
const std::vector<NodeCatalogEntry>& Tga::GetNodeCatalog()
{
	static const std::vector<NodeCatalogEntry> catalog = BuildCatalog();
	return catalog;
}

int Tga::FindCompatiblePin(const NodeCatalogEntry& entry, const NodePinFilter& filter)
{
	if (!filter.active)
		return -1;
...
ScriptNodeTypeId Tga::DrawNodeSearch(bool justOpened, const NodePinFilter& filter, int& outPinIndex, bool& outShowTree)
{
