...
static void RectSelectFrustum(Ag::SelectionRect& aRect, const Ag::Camera &aCamera, const Vector2f &aViewportSize)
{
	const Ag::Matrix4x4f &projection = aCamera.GetProjection();
	Ag::Vector2f resolution{aViewportSize.x, aViewportSize.y};
	...
	Ag::Vector3f tr = Ag::Vector3f { 
		((2.0f * std::max(aRect.start.x, aRect.end.x) / resolution.x) - 1.0f) / projection(1,1),
		(1.0f - (2.0f * std::min(aRect.start.y, aRect.end.y)) / resolution.y) / projection(2,2),
		1.f
	}.GetNormalized();

	Ag::Vector3f bl = Ag::Vector3f { 
		((2.0f * std::min(aRect.start.x, aRect.end.x) / resolution.x) - 1.0f) / projection(1,1),
		(1.0f - (2.0f * std::max(aRect.start.y, aRect.end.y)) / resolution.y) / projection(2,2),
		1.f
	}.GetNormalized();
	//Ag::Vector3f bl = Ag::Vector3f{ tl.x, br.y, 1.f }.GetNormalized();
	//Ag::Vector3f tr = Ag::Vector3f{ br.x, tl.y, 1.f }.GetNormalized();
...
bool RectSelection::CheckFrustum(const Ag::BoxSphereBounds& someBounds)
{
...
void RectSelection::Update(const Vector2f& aMousePos, const Vector2f& aViewportPos, const Vector2f aViewportSize, const Ag::Camera& aCamera)
{
