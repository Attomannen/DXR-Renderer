...
#include <age/animation/Script/AnimationNodes.h>
...
void Ag::Editor::Init(const EditorConfiguration& aEditorConfiguration, std::unique_ptr<EditorGraphicsBase>&& graphics)
{
	...
	Ag::PropertyEditor::RegisterAssetListFunction(ListProjectAssets);
...
bool Ag::Editor::ShowSavePromptModal()
{
...
SceneDocument* Ag::Editor::GetLevelDocument()
{
...
void Ag::Editor::OpenLevel(const fs::path& path)
{
...
void Ag::Editor::OpenLevelNow(const std::string& relativePath)
{
	try
	{
...
void Ag::Editor::OpenStartupLevel()
{
...
std::string Ag::Editor::CreateNewScene(const fs::path& path)
{
	fs::path p = path;
...
std::string Ag::Editor::CreateNewObjectDefinition(const fs::path& path)
{
	fs::path p = path;
	if (p.extension().empty())
...
std::string Ag::Editor::CreateNewMaterial(const fs::path& path)
{
	fs::path p = path;
	if (p.extension().empty())
...
std::string Ag::Editor::CreateNewAnimationClip(const fs::path& path)
{
	fs::path p = path;
	if (p.extension().empty())
...
{
	const std::filesystem::path root = Ag::Settings::GameAssetRoot();
...
void Ag::Editor::Update(float aTimeDelta, InputManager& inputManager)
{
	ImGuizmo::BeginFrame();
