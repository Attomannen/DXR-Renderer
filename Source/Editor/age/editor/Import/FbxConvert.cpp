...
namespace
{
	...
	// absolute path. So they live in <exe folder>/data/AGE_FbxPreview (outside the
	// project, so no asset scan or Content Browser ever sees them) and are referred to
	// as "AGE_FbxPreview/<file>".
	constexpr const char* kPreviewSubfolder = "AGE_FbxPreview";
...
fs::path Ag::FindTextureCookerExe()
{
	wchar_t module[MAX_PATH]{};
	GetModuleFileNameW(nullptr, module, MAX_PATH);
...
// ------------------------------------------------------------------ settings + sidecar
FbxImportSettings Ag::LoadFbxImportSettings(const fs::path& absoluteFbxPath)
{
...
std::string Ag::SaveFbxSidecar(const fs::path& absoluteFbxPath, const FbxImportSettings& settings)
{
...
void Ag::RememberFbxImportDefaults(const fs::path& absoluteFbxPath, const FbxImportSettings& settings)
{
...
std::string Ag::MakeFbxCookRequest(const fs::path& absoluteFbxPath, const FbxImportSettings& settings, FbxCookRequest& outRequest)
{
...
std::string Ag::MakeDefaultFbxCookRequest(const fs::path& absoluteFbxPath, FbxCookRequest& outRequest)
{
	return MakeFbxCookRequest(absoluteFbxPath, LoadFbxImportSettings(absoluteFbxPath), outRequest);
}

// ------------------------------------------------------------------ cooker
std::string Ag::RunFbxCooker(const FbxCookRequest& request)
{
	try
	{
