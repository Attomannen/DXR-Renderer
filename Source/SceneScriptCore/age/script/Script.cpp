...
void Script::WriteToJson(JsonData& result)
{
	...
	for (ScriptNodeId nodeId = GetFirstNodeId(); nodeId.id != ScriptNodeId::InvalidId; nodeId = GetNextNodeId(nodeId))
	{
		...
		Ag::Vector2f pos = GetPosition(nodeId);
...
ScriptNodeId Script::CreateNode(ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& newNode, Ag::Vector2f pos)
{
	UpdateSequenceNumber();
...
void Script::CreateNodeWithReusedId(ScriptNodeId id, ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& newNode, Ag::Vector2f pos)
{
	UpdateSequenceNumber();
...
Ag::Vector2f Script::GetPosition(ScriptNodeId id) const
{
	assert(id.id < myNodes.size());
	assert(myNodes[id.id].isRemoved == false);
...
void Script::SetPosition(ScriptNodeId id, Ag::Vector2f pos)
{
	UpdateSequenceNumber();
