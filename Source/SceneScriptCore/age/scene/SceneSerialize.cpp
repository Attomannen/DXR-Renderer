...
extern void Ag::SaveScene(const Scene& scene, void (*afileChangedCallback)(SceneFileChangeType, const char*))
{
	FilePathStream fullPath;
	fullPath << Ag::Settings::GameAssetRoot() << "/" << scene.GetPath();
...
extern bool Ag::LoadScene(const char* filepath, Scene& scene)
{
	FilePathStream fullPath;
	fullPath << Ag::Settings::GameAssetRoot() << "/" << filepath;
	fullPath.NormalizePath();
